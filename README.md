...
**A Border Tech Solutions product.**

BorderLink is a remote control and remote scripting platform, built with .NET, Blazor, and SignalR Core. It gives IT teams secure, fast, cross-platform access to the endpoints they support — from a single pane of glass.
...
## About Border Tech Solutions

Border Tech Solutions builds practical tooling for the people who keep fleets of computers running. BorderLink is our remote support and lightweight RMM platform — self-hostable, transparent, and built on the modern .NET stack.

## Project Links

Community: https://www.reddit.com/r/remotely_app/  
Container Image: https://hub.docker.com/r/immybot/remotely  
Walkthrough: https://www.youtube.com/watch?v=t-TFvr7sZ6M (Thanks, @bmcgonag!)
...
The BorderLink code does not parse or handle these values. It is done internally by ASP.NET Core's built-in middleware. If the values are not appearing as expected, it is because the headers were missing, didn't contain the correct values, were not in the correct format, or didn't come through a chain of known proxies (see below).

To avoid injection attacks, ASP.NET Core defaults to only accepting forwarded headers from loopback addresses. BorderLink will also add the Docker gateway IP (172.28.0.1) defined in the docker-compose file. If you are using a non-default configuration, you must add all firewall and reverse proxy addresses to the `KnownProxies` array in the Server Config.
...
- Data for BorderLink will be saved in the container under `/app/AppData`, which will be mounted to `/var/www/borderlink/` on your Docker host.
- This folder will contain the BorderLink database and logs generated by the server.
  - These files will persist through teardown and setup of new BorderLink containers.
  - If upgrading from a non-Docker install, overwrite the DB file from your previous installation.
...
The following steps will configure your Windows 11 machine for building the BorderLink server and clients.
...
Within the Account section, there is a tab for branding, which will apply to the quick support clients and Windows installer. This is where downstream customers of Border Tech Solutions can apply their own logos, colors, and product names on top of BorderLink.
...
- KnownProxies: If your reverse proxy is on a different machine and is forwarding requests to the BorderLink server, you will need to add the IP of the reverse proxy server to this array.
...
By default, BorderLink uses a SQLite database. When first run, it creates a file as specified for the SQLite connection string in appsettings.json.

You can change database by changing `DBProvider` in `ApplicationOptions` to `SQLServer` or `PostgreSQL`.

## Logging

- On clients, logs are kept in `%ProgramData%\BorderLink\Logs`
- Within the server container, logs will be written to `/app/AppData/logs`, which (by default) will be mounted to `/var/www/borderlink/Logs` on the host.
...
Ideally, you'd be doing remote control from an actual computer or laptop. However, BorderLink works at least somewhat from a mobile device. Here are the controls:
...
A shortcut to this page is placed in the `\Program Files\BorderLink\` folder. You can copy it anywhere you like. You can also have it copied to the desktop automatically by using the `-supportshortcut` switch on the installer.
...
BorderLink has a basic API, which can be browsed at `https://{your_server_url}/swagger`. Most endpoints require authentication via an API access token, which can be created by going to Account - API Access.

When accessing the API from the browser on another website, you'll need to set up CORS in appsettings by adding the website origin URL to the TrustedCorsOrigins array. If you're not familiar with how CORS works, we recommend reading up on it before proceeding. For example, if you wanted to create a login form on https://example.com that logged into the BorderLink API, you'd need to add "https://example.com" to the TrustedCorsOrigins.
...
The Alerts API gives you the ability to add monitoring and alerting functionality to your device endpoints. This feature is intended to add basic RMM-type functionality without diverging too far from BorderLink's primary purpose.

Alerts can be set up to show a notification on the BorderLink website, send an email, and/or perform a separate API request.
    ...
    if ($FreeSpace -lt .1) {
        Invoke-WebRequest -Uri "https://localhost:5001/api/Alerts/Create/" -Method Post -Headers @{
        ...
        } -Body @"
            {
                ...
                "EmailTo": "support@bordertechsolutions.example",
                "ShouldAlert": true,
                "ShouldEmail": true,
...
&copy; Border Tech Solutions. BorderLink is provided under the project's existing license — see [LICENSE](LICENSE) for details.
