...
if ($SignAssemblies) {
    &"$Root\Utilities\signtool.exe" sign /fd SHA256 /f "$CertificatePath" /p $CertificatePassword /t http://timestamp.digicert.com "$Root\Server\wwwroot\Content\Win-x64\BorderLink_Desktop.exe"
}
...
if ($SignAssemblies) {
    &"$Root\Utilities\signtool.exe" sign /fd SHA256 /f "$CertificatePath" /p $CertificatePassword /t http://timestamp.digicert.com "$Root\Server\wwwroot\Content\Win-x86\BorderLink_Desktop.exe"
}

# Compress Agents.
$PublishDir = "$Root\Agent\bin\publish\win-x64"
Compress-Archive -Path "$PublishDir\*" -DestinationPath "$PublishDir\BorderLink-Win-x64.zip" -Force
Wait-ForExists -FilePath "$PublishDir\BorderLink-Win-x64.zip"
Move-Item -Path "$PublishDir\BorderLink-Win-x64.zip" -Destination "$Root\Server\wwwroot\Content\BorderLink-Win-x64.zip" -Force

$PublishDir = "$Root\Agent\bin\publish\win-x86"
Compress-Archive -Path "$PublishDir\*" -DestinationPath "$PublishDir\BorderLink-Win-x86.zip" -Force
Wait-ForExists -FilePath "$PublishDir\BorderLink-Win-x86.zip"
Move-Item -Path "$PublishDir\BorderLink-Win-x86.zip" -Destination "$Root\Server\wwwroot\Content\BorderLink-Win-x86.zip" -Force

$PublishDir = "$Root\Agent\bin\publish\linux-x64"
Compress-Archive -Path "$PublishDir\*" -DestinationPath "$PublishDir\BorderLink-Linux.zip" -Force
Wait-ForExists -FilePath "$PublishDir\BorderLink-Linux.zip"
Move-Item -Path "$PublishDir\BorderLink-Linux.zip" -Destination "$Root\Server\wwwroot\Content\BorderLink-Linux.zip" -Force

$PublishDir = "$Root\Agent\bin\publish\osx-x64"
Compress-Archive -Path "$PublishDir\*" -DestinationPath "$PublishDir\BorderLink-MacOS-x64.zip" -Force
Wait-ForExists -FilePath "$PublishDir\BorderLink-MacOS-x64.zip"
Move-Item -Path "$PublishDir\BorderLink-MacOS-x64.zip" -Destination "$Root\Server\wwwroot\Content\BorderLink-MacOS-x64.zip" -Force

$PublishDir = "$Root\Agent\bin\publish\osx-arm64"
Compress-Archive -Path "$PublishDir\*" -DestinationPath "$PublishDir\BorderLink-MacOS-arm64.zip" -Force
Wait-ForExists -FilePath "$PublishDir\BorderLink-MacOS-arm64.zip"
Move-Item -Path "$PublishDir\BorderLink-MacOS-arm64.zip" -Destination "$Root\Server\wwwroot\Content\BorderLink-MacOS-arm64.zip" -Force
