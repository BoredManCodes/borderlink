...
/// <summary>
/// Process enumeration via <c>ps -axo pid,ppid,user,rss,comm</c>.
/// </summary>
public class ProcessEnumeratorMac : IProcessEnumerator
{
    private static readonly TimeSpan _timeout = TimeSpan.FromSeconds(20);

    private readonly ILogger<ProcessEnumeratorMac> _logger;

    public ProcessEnumeratorMac(ILogger<ProcessEnumeratorMac> logger)
    {
        _logger = logger;
    }
    ...
    public Task<ProcessInfo[]> GetProcessesAsync(CancellationToken cancellationToken = default)
    {
        return Task.Run(() =>
        {
            var output = RunTool("ps", "-axo pid,ppid,user,rss,comm", cancellationToken);
            ...
            foreach (var rawLine in output.Split('\n'))
            {
                ...
                {
                    first = false;
                    if (line.TrimStart().StartsWith("PID", StringComparison.OrdinalIgnoreCase))
                    {
                        continue;
                    }
                }
    ...
    public Task<bool> KillAsync(int pid, CancellationToken cancellationToken = default)
    {
        return Task.Run(() =>
        {
            try
            {
                var psi = new ProcessStartInfo("kill", $"{pid}")
                {
                    CreateNoWindow = true,
